...
SQL Server 的备份和还原功能是从 Windows 上的 SQL Server 数据库迁移到 Linux 上的 SQL Server 的建议的方法。 在本教程中，将会演练数据库移到 Linux，使用备份和还原技术所需的步骤。
...
1. 在`>1`提示符下，输入以下**RESTORE DATABASE**命令，并在每行 （不能复制并粘贴整个多行命令在一次） 后按 enter 键。 替换所有出现的`YourDB`与数据库的名称。
   ...
   `RESTORE DATABASE` 可能会返回错误，如下例所示：
   ...
   在这种情况下，数据库包含辅助文件。 如果这些文件中未指定`MOVE`子句`RESTORE DATABASE`，还原过程将尝试在与原始服务器相同的路径中创建它们。 
   ...
   可以使用此列表来创建`MOVE`子句为其他文件。 在此示例中，`RESTORE DATABASE`是：
...
在本教程中，您学习了如何在 Windows 上备份数据库并将其移到运行 SQL Server 的 Linux 服务器。 你将了解到：
> [!div class="checklist"]
